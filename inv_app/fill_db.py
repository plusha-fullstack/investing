...
def update_stocks_data_in_db(market_columns, market_data):
    ...
    for data in market_data:
        ...
        try:
            ...
            stock_instance.price = data[last_index] if data[last_index] is not None else 0
            stock_instance.volume_24h = data[valtoday_index] if data[valtoday_index] is not None else 0
            stock_instance.market_cap = data[issuecapitalization_index] if data[issuecapitalization_index] is not None else 0
            stock_instance.save()
    ...
    for data in market_data:
        ...
        try:
            ...
            if last_price == 0:
                ...
                bond_instance.price = last_price
                bond_instance.volume_24h = data[valtoday_index] if data[valtoday_index] is not None else 0
                bond_instance.save()
...
def update_fund_data_in_db(market_columns, market_data):
    ...
    for data in market_data:
        ...
        try:
            ...
            fund_instance.price = data[current_index] if data[current_index] is not None else 0
            fund_instance.volume_24h = data[valtoday_index] if data[valtoday_index] is not None else 0
            fund_instance.market_cap = data[capitalization_index] if data[capitalization_index] is not None else 0
            fund_instance.save()
...
def main():
    ...
    moex_stock_data = fetch_stock_data()
    securities_columns, securities_data, market_columns, market_data = parse_moex_data(moex_stock_data)
    save_stocks_to_db(securities_columns, securities_data)
    update_stocks_data_in_db(market_columns, market_data)

    moex_bonds_data = fetch_bonds_data()
    securities_columns, securities_data, market_columns, market_data = parse_moex_data(moex_bonds_data)
    save_bonds_to_db(securities_columns, securities_data)
    update_bond_data_in_db(market_columns, market_data)

    moex_funds_data = fetch_funds_data()
